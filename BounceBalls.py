...
class BounceBalls:
    def __init__(self):
        ...
        self.canvas = Canvas(
            self.window, bg="white", width=self.width, height=self.height
        )
        self.canvas.pack()
        ...
        btSlower = Button(frame, text="Slower", command=self.speeddown)
        btSlower.pack(side=LEFT)
        btFaster = Button(frame, text="Faster", command=self.speedup)
    ...
    def speeddown(
        self,
    ):  # Speeds down the ball with the help of increasing the sleeptime
        self.sleepTime += 20

    def speedup(self):  # Slows down the ball with the help of decreasing the sleeptime
        self.sleepTime -= 20
