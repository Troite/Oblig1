...
class ComboBoxDemo:
    def __init__(self):
        ...
        var = StringVar()  # StringVar for OptionMenu item values
        var.set("Canada")  # initial value
        comboBox = OptionMenu(
            window, var, "Canada", "China", "US", "UK", command=self.processSelection
        ).pack(fill=BOTH)
    ...
    def processSelection(self, selectedItem):
        ...
        if selectedItem == "Canada":
            ...
            self.canvas.create_image(100, 50, image=self.cnImage, tag="image")
        elif selectedItem == "US":
            self.canvas.create_image(100, 50, image=self.usImage, tag="image")
        elif selectedItem == "UK":
            self.canvas.create_image(100, 50, image=self.ukImage, tag="image")
